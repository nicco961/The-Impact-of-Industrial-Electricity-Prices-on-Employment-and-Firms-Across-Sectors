...
reghdfe Employment_by_group rate_electricity population_pers, absorb(year County_State) cluster(utility)
eststo ev1_manufacturing


reghdfe Establishment_by_group rate_electricity population_pers, absorb(year County_State) cluster(utility)
...
esttab ev1_manufacturing ev3_manufacturing using "C:\Users\nicco\OneDrive\Desktop\Industry occupation electricity prices\results\tables\OLS results\ols_manufacturing.tex", replace tex keep(rate_electricity) p star(* 0.1 ** 0.05 *** 0.01) 



* Export Regression Table with-stage F-statistics
esttab ev1_service ev3_service using "C:\Users\nicco\OneDrive\Desktop\Industry occupation electricity prices\results\tables\OLS results\ols_services.tex", replace tex keep(rate_electricity) p star(* 0.1 ** 0.05 *** 0.01) 



* Export Regression Table with-stage F-statistics
esttab ev1_primary_sector ev3_primary_sector using "C:\Users\nicco\OneDrive\Desktop\Industry occupation electricity prices\results\tables\OLS results\ols_primary_sector.tex", replace tex keep(rate_electricity) p star(* 0.1 ** 0.05 *** 0.01) 

...
ivreghdfe Employment_by_group (rate_electricity = i.post_deregulation c.coal_interaction) population_pers [_aweight = population_1990], absorb(year County_State) cluster(State_id) first
eststo ev1_manufacturing


ivreghdfe Establishment_by_group (rate_electricity = i.post_deregulation  c.coal_interaction) population_pers [_aweight = population_1990], absorb(year County_State) cluster(State_id)
...
esttab ev1_manufacturing ev3_manufacturing using "C:\Users\nicco\OneDrive\Desktop\Industry occupation electricity prices\results\tables\IV results\iv_manufacturing.tex", replace tex keep(rate_electricity) p star(* 0.1 ** 0.05 *** 0.01) 



* Export Regression Table with-stage F-statistics
esttab ev1_service ev3_service using "C:\Users\nicco\OneDrive\Desktop\Industry occupation electricity prices\results\tables\IV results\iv_services.tex", replace tex keep(rate_electricity) p star(* 0.1 ** 0.05 *** 0.01) 



* Export Regression Table with-stage F-statistics
esttab ev1_primary_sector ev3_primary_sector using "C:\Users\nicco\OneDrive\Desktop\Industry occupation electricity prices\results\tables\IV results\iv_primary_sector.tex", replace tex keep(rate_electricity) p star(* 0.1 ** 0.05 *** 0.01) 
